...
blurb varchar(1000),
currency_trailing_code varchar(50),
disable_communication varchar(50));

LOAD DATA INFILE 'C:/EXCELR/P211 crowdfunding/csv data/Crowdfunding_projects_1_rawdata.csv'
INTO TABLE PROJECT
...
create view categorys as select c1.category_id,c1.category_name,c1.parent_id,c2.category_name as 'parent_category_name' 
from category c1 left join category c2 on c1.parent_id=c2.category_id;
...
select *, (category_wise_total/total_count)*100 as '%_of_total' 
from cte 
...
with cte as(select name,backers_count,c.parent_category_name as 'category_name', dense_rank() over(partition by c.parent_category_name order by backers_count desc) as 'dn'
		from categorys c right join projects p on p.category_id=c.category_id
		where state='successful')
select name , category_name,backers_count as 'Backers_count' from cte where dn<=2;
...
with cte as(select name,usd_pledged,c.parent_category_name as 'category_name', dense_rank() over(partition by c.parent_category_name order by usd_pledged desc) as 'dn'
		from categorys c right join projects p on p.category_id=c.category_id
		where state='successful')
...
with cte as(select name,usd_pledged,c.parent_category_name as 'category_name',a.country as 'Country' ,a.code as 'code', dense_rank() over(order by usd_pledged desc) as 'dn'
		from categorys c right join projects p on p.category_id=c.category_id left join country a on p.country=a.code
		where state='successful')
select name , category_name,country,code,usd_pledged as 'Amount raised' from cte where dn<=1;

#----------------------Highest amount raised for successful projects based on backers count with category,loaction--------------------------------------#

with cte as(select name,backers_count,c.parent_category_name as 'category_name',a.country as 'Country' ,a.code as 'code', dense_rank() over(order by backers_count desc) as 'dn'
		from categorys c right join projects p on p.category_id=c.category_id left join country a on p.country=a.code
		where state='successful')
select name , category_name,country,code,backers_count as 'Backers count' from cte where dn<=1;